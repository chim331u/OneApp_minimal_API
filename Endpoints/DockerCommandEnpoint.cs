...
public static class DockerCommandEnpoint
{
    ...
    public static IEndpointRouteBuilder MapDockerCommandEndPoint(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPost("/SendSSHCommand/{id:int}",
            async (int id, string command, IDockerCommandService _dockerCommandService) =>
            {
                var commandResult = await _dockerCommandService.SendSSHCommand(id, command);
                return Results.Ok(commandResult);
            });

        app.MapPost("/SendSSHCommand",
            async (DockerCommandRequestDto commandRequest, IDockerCommandService _dockerCommandService) =>
            {
                var commandResult =
                    await _dockerCommandService.SendSSHCommand(commandRequest.DockerId, commandRequest.Command);
        ...
        /// <param name="_dockerCommandService">The service to process the command.</param>
        app.MapPost("/SendLocalhostCommand/{id:int}",
            async (int id, string command, IDockerCommandService _dockerCommandService) =>
            {
        ...
        app.MapPost("/SendLocalhostCommand",
            async (DockerCommandRequestDto commandRequest, IDockerCommandService _dockerCommandService) =>
            {
                var commandResult =
                    await _dockerCommandService.SendLocalhostCommand(commandRequest.DockerId,
        ...
        app.MapGet("/GetRemoteRunningContainers/{id:int}",
            async (int id, IDockerCommandService _dockerCommandService) =>
            {
                var commandResult = await _dockerCommandService.GetRemoteRunningContainers(id);
                return Results.Ok(commandResult);
            });

        app.MapGet("/RemoveRemoteRunningContainers/{id:int}",
            async (int id, IDockerCommandService _dockerCommandService) =>
            {
        ...
        app.MapGet("/ExecuteFullDeploy/{id:int}", async (int id, IHangFireJobService jobService) =>
        {
            var jobId = BackgroundJob.Enqueue<IHangFireJobService>(job =>
                job.ExecuteFullDeploy(id, CancellationToken.None));
        ...
        app.MapGet("/GetLoginDockerRegistryCommand/{id:int}",
            async (int id, IDockerCommandService _dockerCommandService) =>
            {
