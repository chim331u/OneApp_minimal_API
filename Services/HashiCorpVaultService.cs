...
public class HashiCorpVaultService : IHashicorpVaultService
{
    ...
    private string _vaultToken; //= ""; // Example token, replace with your actual token
    
    private const string VaultToken = "VAULT_TOKEN";
    ...
    public HashiCorpVaultService(IConfiguration configuration, ILogger<HashiCorpVaultService> logger)
    {
        ...
        _logger = logger;
        //for debug only
        _vaultToken = _configuration.GetSection("IsDev").Value != null ? configuration[VaultToken] : Environment.GetEnvironmentVariable(VaultToken);
    ...
    public async Task<ApiResponse<SecretResponseDTO>> GetSecret(string key, string path, string mountPoint)
    {
        try
        {
            ...
            if (kv2Secret.Data.Data.TryGetValue(key, out var value))
            {
                ...
                return new ApiResponse<SecretResponseDTO>( new SecretResponseDTO { Key = key, Value = value.ToString() }, $"Secret found in Vault at path '{path}'");
            }
    ...
    public async Task<ApiResponse<List<string>?>> GetListSecretsKeys(string path, string mountPoint)
    {
        try
        {
            ...
            if (kv2Secret.Data.Data.Keys.Any())
            {
                ...
                return new ApiResponse<List<string>?>(kv2Secret.Data.Data.Keys.ToList(), $"Found '{kv2Secret.Data.Data.Keys.Count}' keys in Vault at path '{path}'");
            }
    ...
    public async Task<ApiResponse<SecretResponseDTO>> CreateNewSecret(SecretRequestDTO secret)
    {
        try
        {
            ...
            var writtenValue = await vaultClient.V1.Secrets.KeyValue.V2.WriteSecretAsync(secret.Path, newSecret, null, secret.MountVolume);

            return new ApiResponse<SecretResponseDTO>(new SecretResponseDTO(){Key = secret.Key, Value = secret.Value.ToString()}, $"New secret added at path '{secret.Path}' with key '{secret.Key}'");
    ...
    public async Task<ApiResponse<SecretResponseDTO>> CreateSecret(SecretRequestDTO secret)
    {
        try
        {
            ...
            var metadata = await vaultClient.V1.Secrets.KeyValue.V2.PatchSecretAsync(secret.Path, patchSecretDataRequest, mountPoint: secret.MountVolume);
    ...
    public async Task<ApiResponse<SecretResponseDTO>> UpdateSecret(SecretRequestDTO secret)
    {
        try
        {
            ...
            var patchSecretDataRequest = new PatchSecretDataRequest() { Data = valueToBeCombined };
            var metadata = await vaultClient.V1.Secrets.KeyValue.V2.PatchSecretAsync(secret.Path, patchSecretDataRequest, mountPoint: secret.MountVolume);
            return new ApiResponse<SecretResponseDTO>(new SecretResponseDTO(){Key = secret.Key, Value = secret.Value.ToString()}, $"Secret updated at path '{secret.Path}' with key '{secret.Key}'");
